...
alert("用户添加失败！");
	...
	<div class="menu">
		...
		<table>
			<tbody>
				<tr>
					<td><form method="post" action="UserServlet?cmd=getUser">
				...
				<tbody>
					<tr>
						...
						<c:if test="${sessionScope.user.type == 0 }">
							<td width="150"><div class="STYLE1" align="center">操作
								</div></td>
						</c:if>
					</tr>
					<c:forEach items="${pagebean.data }" var="user">
						...
						<td><span class="STYLE1"><a
								href="UserServlet?cmd=retrievalUser&userId=${user.userId }">修改</a>&emsp;<a
								href="UserServlet?cmd=delUser&userId=${user.userId }">删除</a></span></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
			共${pagebean.count }条记录,&nbsp第${pagebean.p }页/共${pagebean.pagetotal
			}页&nbsp <a href="UserServlet?cmd=selectUsers&p=1&addd=yes">首页</a>&nbsp
			<a href="UserServlet?cmd=selectUsers&p=${pagebean.p-1 }">上一页</a>&nbsp
			<a href="UserServlet?cmd=selectUsers&p=${pagebean.p+1 }">下一页</a>&nbsp
			<a href="UserServlet?cmd=selectUsers&p=${pagebean.pagetotal }">尾页</a>&nbsp跳到第
	...
	<script type="text/javascript">
function toPage(a){
	location="UserServlet?cmd=selectUsers&p="+a;
}
function setP(){
