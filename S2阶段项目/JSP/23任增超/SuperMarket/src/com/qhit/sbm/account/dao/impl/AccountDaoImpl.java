...
public class AccountDaoImpl implements AccountDao {
	...
	public PageBean getPageBean(PageBean pageBean) {
		...
		String sql = "select top " + pageBean.getPagesize() + " a.accountId,g.goodsName,g.goodsNum,a.totalPrice,"
				+ " a.isPayed,p.providerName,g.goodsIntro,a.accountDate "
				+ " from tb_account a left join tb_goods g on a.goodsId = g.goodsId "
				+ " left join tb_provider p on p.providerId = a.providerId  " + " where a.accountId not in (select top "
		...
		try {
			...
			while (rs.next()) {
				...
				accountDetail = new AccountDetail(rs.getInt("accountId"), rs.getString("goodsName"),
						rs.getInt("goodsNum"), rs.getInt("businessNum"), rs.getFloat("totalPrice"),
						rs.getInt("isPayed"), rs.getString("providerName"), rs.getString("goodsIntro"),
	...
	public PageBean getPageBeanByParam(List<String> wheres, List<String> values) {
		...
		AccountDetail accountDetail = null;
		String sql = "select a.accountId,g.goodsName,g.goodsNum,a.businessNum,"
				+ "a.totalPrice, a.isPayed,p.providerName," + "g.goodsIntro,a.accountDate "
				+ "from tb_account a left join " + "tb_goods g on a.goodsId = g.goodsId  " + "left join tb_provider p "
		...
		try {
			...
			/**
			 * pagesize count pagetotal p data 第一页显示出所有的查询结果
			 */
